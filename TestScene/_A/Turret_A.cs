...
// 목적지
// 나의체력
public enum turretAState {
    Idle,
    Attack,
}
public class Turret_A : MonoBehaviour {
    public turretAState state;
    ...
    HP_A hp;
    TurretDetection_A turretDetection;
    ...
    void Awake () {
        ...
        turretDetection = GetComponentInChildren<TurretDetection_A>();
    }
    void Start () {
        Initialize ();
    }
    void FixedUpdate () {
        ...
        //타겟이 있으면
        if (state == turretAState.Attack && turretDetection.targetIsDetected) {
            if(target)
            {
