...
// 목적지
// 나의체력
public enum unit1State {
    MoveAndSearch,
    Attack,
}
public class Unit1_8 : MonoBehaviour {

    public unit1State state;
    ...
    Unit1Detection_8 unitDetection;
    HP_8 hp;
    ...
    void FixedUpdate () {
        ...
        if (target) {
            //타겟이 시야 내 감지되었으면
            if (state == unit1State.MoveAndSearch) {
                Move (target.position);
                //SearchTarget ();
            } else if (state == unit1State.Attack && unitDetection.targetIsDetected) {
    ...
    void Initialize () {
        state = unit1State.MoveAndSearch;
        target = goal;
        viewDistance = 5.0f;
