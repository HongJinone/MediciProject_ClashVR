...
// 목적지
// 나의체력
public enum unit2_10State {
    MoveAndSearch,
    Attack,
}
public class Unit2_10 : MonoBehaviour {

    public unit2_10State state;
    ...
    Unit2Detection_10 unitDetection;
    HP_8 hp;
    ...
    void Awake () {
        ...
        unitDetection = GetComponentInChildren<Unit2Detection_10>();
    ...
    void FixedUpdate () {
        ...
        if (target) {
            //타겟이 시야 내 감지되었으면
            if (state == unit2_10State.MoveAndSearch) {
                Move (target.position);
                //SearchTarget ();
            } else if (state == unit2_10State.Attack && unitDetection.targetIsDetected) {
    ...
    void Initialize () {
        state = unit2_10State.MoveAndSearch;
